...
import { UserInfo } from '../pages/auth/models/user-info.interface';
...
export class AuthService {
  ...
  askUpdatePassword(): Observable<any> {
    return this.httpClient.get<any>("http://localhost:3000/auth/update_password/");
  }

  updateUser(personalInfo: any): Observable<any> {
    return this.httpClient.put<any>("http://localhost:3000/auth/update_user", personalInfo);
  } 
  ...
  autoLogin(){
    ...
    const loadedUser = new User(userData.lastname, userData.firstname,userData.email, userData.id, userData._accesstoken, new Date(userData._tokenExpirationDate))
    if(loadedUser.token){
